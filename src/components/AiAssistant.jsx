    ...
    return (
        <>
            <button
                ...
                className={`absolute bottom-3 md:bottom-6 right-3 md:right-6 z-[1000] bg-[#38bdf8] text-white p-3 md:p-4 rounded-full shadow-[0_0_20px_rgba(56,189,248,0.4)] hover:scale-110 transition-transform ${isOpen ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'}`}
            >
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute bottom-3 md:bottom-6 right-3 md:right-6 z-[1010] w-[280px] md:w-[350px] bg-[#0b1219]/90 backdrop-blur-xl border border-[#38bdf8]/30 rounded-2xl md:rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[85%]"
                    >
                        {/* Header */}
                        ...
                        <div className="flex-1 max-h-[200px] md:max-h-[250px] overflow-y-auto p-4 flex flex-col gap-3 scrollbar-hide" style={{ scrollbarWidth: 'none' }}>
                            {messages.map((msg, i) => (
                                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[85%] p-3 rounded-2xl text-[10px] md:text-xs leading-relaxed font-medium shadow-sm ${msg.role === 'user' ? 'bg-[#38bdf8]/90 text-black rounded-tr-sm font-bold' : 'bg-white/5 border border-white/10 text-neutral-300 rounded-tl-sm'}`}>
                                        {msg.text}
                                    </div>
                                </div>
                            ))}
                            {isTyping && (
                                <div className="flex justify-start">
                                    <div className="bg-white/5 border border-white/10 text-neutral-300 px-3 py-4 rounded-2xl rounded-tl-sm flex gap-1.5 items-center backdrop-blur-md">
                                        <div className="w-1.5 h-1.5 rounded-full bg-[#38bdf8] animate-bounce"></div>
                                        <div className="w-1.5 h-1.5 rounded-full bg-[#38bdf8] animate-bounce" style={{ animationDelay: '0.15s' }}></div>
                                        <div className="w-1.5 h-1.5 rounded-full bg-[#38bdf8] animate-bounce" style={{ animationDelay: '0.3s' }}></div>
                                    </div>
                                </div>
                            )}
                        ...
                        <form onSubmit={handleSend} className="p-3 bg-black/40 border-t border-white/10 relative">
                            <div className="relative group">
                                <input
                                    ...
                                    disabled={isTyping}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl py-2 md:py-2.5 pl-3 pr-10 text-[10px] md:text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-[#38bdf8]/50 transition-colors shadow-inner disabled:opacity-50 disabled:cursor-not-allowed"
                                />
                                <button type="submit" disabled={!input.trim() || isTyping} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#38bdf8] hover:text-white disabled:opacity-50 disabled:hover:text-[#38bdf8] transition-colors p-1">
                                    <Send size={14} />
                                </button>
                            </div>
                        </form>
